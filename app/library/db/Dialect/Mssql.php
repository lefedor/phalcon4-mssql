...
class Mssql extends Dialect
{
    ...
    */
	...
	public function select(array $definition) : string // 2.x
    {
		...
		if (isset($limitValue)) {
			if (is_array($limitValue)) {
				...
				if (isset($limitValue['offset'])) {
					...
					$offset = $limitValue['offset']['value'];
					$number = $limitValue['number']['value'];
					...
					//$sql = 'SELECT * FROM ( '.$sql.' ) subq OFFSET '.$offset.' ROWS FETCH NEXT '.$number.' ROWS ONLY ';
				} else {
	...
	public function getColumnDefinition(\Phalcon\Db\ColumnInterface $column) : string // 2.x
    {
        $columnSql;
        $size;
        $scale;
	...
	public function addColumn($tableName, $schemaName, \Phalcon\Db\ColumnInterface $column) : string // 2.x
    {
        $afterPosition;
        $sql;
	...
	public function modifyColumn($tableName, $schemaName, \Phalcon\Db\ColumnInterface $column, \Phalcon\Db\ColumnInterface $currentColumn = NULL) : string // 2.x
    {
        $sql;
	...
	public function addIndex($tableName, $schemaName, \Phalcon\Db\IndexInterface $index) : string // 2.x
    {
        $sql;
        if (!is_object($index)) {
	...
	public function addPrimaryKey($tableName, $schemaName, \Phalcon\Db\IndexInterface $index) : string // 2.x
    {
        $sql;
        if (!is_object($index)) {
            throw new Phalcon\Db\Exception("Index parameter must be an object compatible with Phalcon\\Db\\IndexInterface");
	...
	public function createView(string $viewName, array $definition, $schemaName = NULL) : string  // 2.x
    {
        $view;
        $viewSql;
	...
	public function dropView(string $viewName, $schemaName = NULL, $ifExists = NULL) : string // 2.x
    {
        $sql="";
        $view;
    ...
    public function tableOptions(string $table, $schema = null) : string
    {
       ...
       // public function addForeignKey($tableName, $schemaName, $reference){} // 1.x
		public function addForeignKey(string $tableName, $schemaName, \Phalcon\Db\ReferenceInterface $reference) : string {
			return '';//TODO
		}
		public function dropForeignKey(string $tableName, $schemaName, $referenceName) : string {
			return '';//TODO
		}

       // public function createTable($tableName, $schemaName, $definition){} // 1.x
	   public function createTable(string $tableName, string $schemaName, array $definition) : string {} // 2.x
       public function dropTable(string $tableName, string $schemaName) : string {}
		
       public function supportsSavepoints() : bool { return false; }
       public function supportsReleseSavepoints() : bool { return false; }
